 ...
 *
 * Lane 2 – SCIM → Convex
 *   Directly exercise the Convex enterprise SCIM server using plain HTTP with
...
import {
  ...
  enterpriseWebhookDeliveryListRpc,
  enterpriseWebhookEndpointCreateRpc,
  randomSlug,
  requireEnv,
  requestHttp,
  requestJson,
...
type EnterpriseWebhookEndpoint = {
  _id?: string;
  enterpriseId?: string;
  url?: string;
  status?: string;
...
// ---------------------------------------------------------------------------
// Gate — only run when ZITADEL_INTEROP_TEST is enabled and PATs are injected
// ---------------------------------------------------------------------------
...
maybeInterop(
  "SCIM → ZITADEL: provision and manage user lifecycle",
  async () => {
...
maybeInterop(
  "SCIM → Convex: direct SCIM server protocol validation",
  async () => {
    ...
    // Create an enterprise and configure SCIM to get a bearer token
    ...
    const webhookCreated = (await enterpriseWebhookEndpointCreateRpc(
      convexClient,
      convexUserToken,
      {
        enterpriseId,
    ...
    const base = `${convexSiteUrl}/api/auth/sso/${enterpriseId}/scim/v2`;
    ...
    const secondUserRes = await scimRequest<ScimUser>(
      base,
      "/Users",
      scimToken,
      {
    ...
    expect(
      auditEvents.some(
        (event) =>
          event.eventType === "enterprise.scim.user.updated" &&
          event.subjectId === userId,
      ),
    ).toBe(true);
    expect(
      auditEvents.some(
        (event) =>
          event.eventType === "enterprise.scim.user.deleted" &&
          event.subjectId === userId,
      ),
    ).toBe(true);
    expect(
      auditEvents.some(
        (event) =>
          event.eventType === "enterprise.scim.group.created" &&
    ...
    expect(
      auditEvents.some(
        (event) =>
          event.eventType === "enterprise.scim.group.updated" &&
          event.subjectId === groupId,
      ),
    ).toBe(true);

    convexClient.setAuth(convexUserToken);
    const webhookEndpoints = (await convexClient.action(
      (api as any).enterprise.webhook.endpoint.list,
      { enterpriseId },
    )) as EnterpriseWebhookEndpoint[];
    expect(
    ...
    const webhookDeliveries = (await enterpriseWebhookDeliveryListRpc(
      convexClient,
      convexUserToken,
      { enterpriseId, limit: 50 },
    )) as EnterpriseWebhookDelivery[];
    expect(
      webhookDeliveries.some(
        (delivery) =>
          delivery.eventType === "enterprise.scim.configured" &&
    ...
    expect(
      webhookDeliveries.some(
        (delivery) =>
          delivery.eventType === "enterprise.scim.user.created" &&
          delivery.auditEventId,
      ),
    ).toBe(true);
    expect(
      webhookDeliveries.some(
        (delivery) =>
          delivery.eventType === "enterprise.scim.group.updated" &&
          delivery.auditEventId,
      ),
    ).toBe(true);
  },
