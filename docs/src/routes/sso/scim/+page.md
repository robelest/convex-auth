...
| ----------- | --------------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------- |
| `configure` | `(ctx, { enterpriseId, basePath?, status? })` | `{ enterpriseId, token, configId, basePath }` | Configures SCIM provisioning and returns the SCIM bearer token once.                  |
| `get`       | `(ctx, enterpriseId)`                         | SCIM config document                          | Returns the current SCIM configuration for a connection.                              |
| `validate`  | `(ctx, enterpriseId)`                         | `{ checks: [...] }`                           | Validates that the SCIM configuration is complete. Each check has its own `ok` field. |
