...
| ----------- | ---------------------------------------------- | -------------------- | --------------------------------------------------------------------------- |
| `create`    | `(ctx, { name, parentId?, tags?, metadata? })` | `{ ok, groupId }`    | Creates a new group. Optionally nest under a parent group.                  |
...
| `update`    | `(ctx, groupId, { name?, tags?, metadata? })`  | `{ ok, groupId }`    | Updates a group's name, tags, or metadata.                                  |
| `delete`    | `(ctx, groupId)`                               | `{ ok, groupId }`    | Deletes a group and all its nested children, members, and invites.          |
