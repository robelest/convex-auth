...
| `list`    | `(ctx, { groupId?, userId?, limit?, cursor? })`                       | `PaginationResult<Doc<"GroupMember">>` — `{ page, isDone, continueCursor }` | Lists members by group, by user, or both. Convex-native shape. |
