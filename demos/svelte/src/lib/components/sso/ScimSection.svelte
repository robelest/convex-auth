<script lang="ts">
  ...
  async function handleEnable() {
    isEnabling = true;
    try {
      const result = await client.mutation(api.auth.group.setScim, { connectionId });
      scimCredentials = result;
    } catch (e: unknown) {
      toast.error(errorText(e, "Failed to enable SCIM"));
    } finally {
      isEnabling = false;
    }
  }
...
<div class="flex flex-col gap-4">
  ...
  {:else if isConfigured}
    <div class="border border-border-transparent bg-background-secondary p-6 flex flex-col gap-4">
      <div class="flex items-center gap-3">
        <p class="m-0 font-label text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-content-secondary">SCIM Provisioning</p>
      ...
      <label class="flex flex-col gap-1.5">
        <span class="font-label text-xs font-semibold text-content-primary">Endpoint</span>
        <input class="input font-mono text-xs" value={displayBasePath} readonly />
      </label>

      {#if displayToken}
        <div class="flex flex-col gap-1.5">
          <span class="font-label text-xs font-semibold text-content-primary">Bearer Token</span>
