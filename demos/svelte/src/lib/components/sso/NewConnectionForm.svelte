<script lang="ts">
  ...
  async function handleCreateConnection() {
    ...
    try {
      ...
      toast.error(errorText(e, "Failed to create connection"));
    } finally {
      isSubmitting = false;
    }
  }
  ...
  async function handleConfigureOidc() {
    ...
    try {
      ...
      toast.error(errorText(e, "Failed to configure OIDC"));
    } finally {
      isSubmitting = false;
    }
  }
  ...
  async function handleConfigureSaml() {
    ...
    try {
      ...
      toast.error(errorText(e, "Failed to configure SAML"));
    } finally {
      isSubmitting = false;
    }
  }
  ...
  async function handleCopyMetadataXml() {
    ...
    try {
      ...
      toast.error(errorText(e, "Failed to copy metadata XML"));
    }
  }
</script>
...
<div class="flex flex-col gap-6">
    ...
    {#each wizardSteps as wizardStep, index (wizardStep)}
      <div class="flex items-center gap-3 flex-1 {index > 0 ? 'pl-4' : ''}">
        <div class="flex items-center justify-center w-8 h-8 border-2 font-label text-sm font-bold shrink-0
          {index < currentStepIndex ? 'border-green-600 bg-green-50 text-green-700' : index === currentStepIndex ? 'border-brand-red bg-brand-red text-white' : 'border-border-transparent bg-background-tertiary text-content-tertiary'}">
        ...
        <span class="font-label text-[0.8125rem] font-semibold {index === currentStepIndex ? 'text-content-primary' : index < currentStepIndex ? 'text-green-700' : 'text-content-tertiary'}">{wizardStep}</span>
        {#if index < wizardSteps.length - 1}
          <div class="flex-1 h-px bg-border-transparent ml-3"></div>
        {/if}
      </div>
    {/each}
  </div>
  ...
  {#if step === "info"}
    <form class="border border-border-transparent bg-background-secondary p-6 flex flex-col gap-5" onsubmit={(e) => { e.preventDefault(); handleCreateConnection(); }}>
      <label class="flex flex-col gap-1.5">
        <span class="font-label text-xs font-semibold text-content-primary">Connection name</span>
        <input bind:value={name} class="input" type="text" placeholder="ACME Corp SSO" required />
        <span class="font-label text-[0.72rem] text-content-tertiary italic">A descriptive name for this SSO connection</span>
      </label>

      <label class="flex flex-col gap-1.5">
        <span class="font-label text-xs font-semibold text-content-primary">User email domain</span>
        <input bind:value={emailDomain} class="input" type="text" placeholder="acme.com" />
        <span class="font-label text-[0.72rem] text-content-tertiary italic">Email domain of users who sign in via this connection</span>
      </label>

      <!-- IdP selector as cards -->
      <div class="flex flex-col gap-1.5">
        <span class="font-label text-xs font-semibold text-content-primary">Identity Provider</span>
          ...
          {#each idpTemplates as tpl (tpl.id)}
            <button
              ...
              class="relative flex flex-col items-center gap-2 border px-4 py-4 cursor-pointer transition-colors text-center
                {providerId === tpl.id ? 'border-brand-red bg-brand-red/5' : 'border-border-transparent bg-background-secondary hover:bg-background-secondary'}"
              onclick={() => { providerId = tpl.id; }}
            >
              <span class="font-label text-[0.8125rem] font-semibold {providerId === tpl.id ? 'text-brand-red' : 'text-content-primary'}">{tpl.label}</span>
              <span class="font-label text-[0.625rem] text-content-tertiary">{tpl.protocols.map((p) => p.toUpperCase()).join(' / ')}</span>
              {#if providerId === tpl.id}
                <div class="absolute top-0 left-0 w-full h-[2px] bg-brand-red"></div>
              {/if}
            </button>
          {/each}
        </div>
      ...
      <label class="flex flex-col gap-1.5">
        <span class="font-label text-xs font-semibold text-content-primary">Protocol</span>
  ...
  {:else if step === "oidc"}
    <form class="border border-border-transparent bg-background-secondary p-6 flex flex-col gap-5" onsubmit={(e) => { e.preventDefault(); handleConfigureOidc(); }}>
      <!-- SP values callout -->
      <div class="border border-border-transparent bg-background-tertiary p-4 flex flex-col gap-3">
        <p class="m-0 font-label text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-content-secondary">Copy to your Identity Provider</p>
        {#if oidcCallbackUrl}
          <div class="flex items-center gap-3 min-w-0 max-md:flex-col max-md:items-start max-md:gap-1">
            ...
            <code class="flex-1 font-mono text-[0.72rem] text-content-primary break-all min-w-0">{oidcCallbackUrl}</code>
      ...
      <div class="flex flex-col gap-4">
        <p class="m-0 font-label text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-content-secondary">Paste from your Identity Provider</p>
        {#each selectedTemplate.oidcFields as field (field.key)}
          <label class="flex flex-col gap-1.5">
            <span class="font-label text-xs font-semibold text-content-primary">{field.label}</span>
            <div class="flex items-center min-w-0">
              ...
              />
              {#if field.suffix}
                <span class="py-2 px-3 border border-border-transparent border-l-0 bg-background-tertiary font-label text-xs text-content-secondary whitespace-nowrap">{field.suffix}</span>
              {/if}
            </div>
          </label>
        {/each}
        <p class="m-0 font-label text-[0.72rem] text-content-tertiary italic">{selectedTemplate.helpText}</p>
      </div>
  ...
  {:else if step === "samlApp"}
    <div class="border border-border-transparent bg-background-secondary p-6 flex flex-col gap-5">
      <div class="border border-border-transparent bg-background-tertiary p-5 flex flex-col gap-4">
        <div class="flex items-center justify-between gap-3 flex-wrap">
          <p class="m-0 font-label text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-content-secondary">Copy these values into your Identity Provider</p>
          {#if connectionId}
        ...
        {#if samlEntityId && samlAcsUrl && samlSloUrl}
          <div class="flex flex-col gap-3">
            {#each [
              ...
              <div class="flex items-center gap-3 min-w-0 max-md:flex-col max-md:items-start max-md:gap-1">
                <span class="font-label text-[0.625rem] font-semibold uppercase tracking-[0.1em] text-content-tertiary w-20 shrink-0">{item.label}</span>
                <code class="flex-1 font-mono text-[0.72rem] text-content-primary break-all min-w-0">{item.value}</code>
            ...
            {#if samlRuntimeMetadataUrl}
              <div class="border-t border-border-transparent pt-3 flex flex-col gap-1">
                <span class="font-label text-[0.625rem] font-semibold uppercase tracking-[0.1em] text-content-tertiary">Runtime metadata URL</span>
                <p class="m-0 font-label text-[0.72rem] text-content-tertiary italic">Serves the hosted metadata document after activation.</p>
                <div class="flex items-center gap-3 min-w-0 max-md:flex-col max-md:items-start max-md:gap-1">
                  <code class="flex-1 font-mono text-[0.72rem] text-content-primary break-all min-w-0">{samlRuntimeMetadataUrl}</code>
  ...
  {:else}
    <form class="border border-border-transparent bg-background-secondary p-6 flex flex-col gap-5" onsubmit={(e) => { e.preventDefault(); handleConfigureSaml(); }}>
      <div class="flex flex-col gap-4">
        <p class="m-0 font-label text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-content-secondary">Paste metadata from your Identity Provider</p>
        <label class="flex flex-col gap-1.5">
      ...
      <div class="flex flex-col gap-4 border-t border-border-transparent pt-5">
        <p class="m-0 font-label text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-content-secondary">Attribute mapping</p>
        ...
        <label class="flex items-center gap-2">
          ...
          <span class="font-label text-[0.75rem] text-content-primary">Sign AuthnRequests</span>
        </label>
      </div>
