...
const failedResponse: string = String(
  readFileSync("./test/misc/failed_response.xml"),
);
...
const parseRedirectUrlContextCallBack = (_context: string) => {
  ...
  const _octetString = Object.keys(originalURL.query)
...
const noSignedIdpMetadata = readFileSync("./test/misc/idpmeta_nosign.xml")
  .toString()
  .trim();
const spmetaNoAssertSign = readFileSync("./test/misc/spmeta_noassertsign.xml")
  .toString()
  .trim();
...
test("create login request with redirect binding using default template and parse it", async () => {
  ...
  const octetString = Object.keys(originalURL.query)
    .map((q) => q + "=" + encodeURIComponent(originalURL.query[q] as string))
    .join("&");
  const { samlContent, extract } = await idp.parseLoginRequest(sp, "redirect", {
...
test("create login request with redirect binding using [custom template]", () => {
  ...
  id === "exposed_testing_id" && isString(context)
...
test("create login request with post binding using [custom template]", () => {
  ...
  const { id, context, entityEndpoint, type, relayState } =
    _sp.createLoginRequest(idp, "post", (template) => {
      return {
        id: "exposed_testing_id",
        context: template, // all the tags are supposed to be replaced
      };
...
test("create login request with post simpleSign binding using [custom template]", () => {
  ...
  const { id, context, entityEndpoint, type, relayState, signature, sigAlg } =
    _sp.createLoginRequest(idp, "simpleSign", (template) => {
      return {
        id: "exposed_testing_id",
        context: template, // all the tags are supposed to be replaced
      };
...
test("create post simpleSign login response", async () => {
  const user = { email: "user@esaml2.com" };
  const { id, context, entityEndpoint, type, signature, sigAlg } =
...
test("create logout request with post binding", () => {
  const { relayState, type, entityEndpoint, id, context } =
    sp.createLogoutRequest(idp, "post", {
      logoutNameID: "user@esaml2",
    }) as PostBindingContext;
  isString(id) &&
  isString(context) &&
...
test("idp sends a redirect logout request without signature and sp parses it", async () => {
  ...
  const SAMLRequest = encodeURIComponent(
    originalURL.query.SAMLRequest as string,
  );
  let result;
...
test("Customize prefix (saml2) for encrypted assertion tag", async () => {
  ...
  const { id, context: SAMLResponse } =
    await idpCustomizePfx.createLoginResponse(
      sp,
      sampleRequestInfo,
      "post",
      user,
...
test.sequential(
  "should throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response without clock drift setup",
  async () => {
    const now = new Date();
...
test.sequential(
  "should throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response by redirect without clock drift setup",
  async () => {
    const now = new Date();
    const fiveMinutesOneSecLater = new Date(now.getTime());
    fiveMinutesOneSecLater.setMinutes(fiveMinutesOneSecLater.getMinutes() + 5);
...
test.sequential(
  "should throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response by simpleSign without clock drift setup",
  async () => {
    const now = new Date();
...
test.sequential(
  "should not throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response with clock drift setup",
  async () => {
    const now = new Date();
    const fiveMinutesOneSecLater = new Date(now.getTime() + 301_000);
    const user = { email: "user@esaml2.com" };
...
test.sequential(
  "should not throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response by redirect with clock drift setup",
  async () => {
    const now = new Date();
    const fiveMinutesOneSecLater = new Date(now.getTime() + 301_000);
    const user = { email: "user@esaml2.com" };
...
test.sequential(
  "should not throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response by simpleSign with clock drift setup",
  async () => {
    const now = new Date();
    const fiveMinutesOneSecLater = new Date(now.getTime() + 301_000);
    const user = { email: "user@esaml2.com" };
