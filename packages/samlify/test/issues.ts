...
(() => {
  ...
  const sp98 = serviceProvider({
    metadata: fs.readFileSync("./test/misc/sp_metadata_98.xml"),
  });
  ...
  test("#86 duplicate issuer throws error", () => {
    ...
    expect(issuer.every((i) => i === "http://www.okta.com/dummyIssuer")).toBe(
      true,
    );
  });
  ...
  test("#87 add existence check for signature verification", () => {
    const res = libsaml.verifySignature(
      readFileSync("./test/misc/response.xml").toString(),
      {},
    );
