...
const spCertKnownGood = readFileSync("./test/key/sp/knownGoodCert.cer")
  .toString()
  .trim();
const spPemKnownGood = readFileSync("./test/key/sp/knownGoodEncryptKey.pem")
  .toString()
  .trim();
...
test("parse cer format resulting clean certificate", () => {
  expect(
    utility.normalizeCerString(readFileSync("./test/key/sp/cert.cer")),
...
(() => {
  const _originRequest: string = String(
    readFileSync("./test/misc/request.xml"),
  );
  const _decodedResponse: string = String(
    readFileSync("./test/misc/response_signed.xml"),
  );
  const _falseDecodedRequestSHA1: string = String(
    readFileSync("./test/misc/false_signed_request_sha1.xml"),
  );
  const _decodedRequestSHA256: string = String(
    readFileSync("./test/misc/signed_request_sha256.xml"),
  );
  const _falseDecodedRequestSHA256: string = String(
    readFileSync("./test/misc/false_signed_request_sha256.xml"),
  );
  const _decodedRequestSHA512: string = String(
    readFileSync("./test/misc/signed_request_sha512.xml"),
  );
  const _falseDecodedRequestSHA512: string = String(
    readFileSync("./test/misc/false_signed_request_sha512.xml"),
  );
  ...
  const octetStringSHA256: string =
  ...
  const octetStringSHA512: string =
  ...
  const signatureB64SHA512: string =
  ...
  const signatureB64SHA256: string =
  ...
  const dummySignRequestSHA256: string =
  ...
  const dummySignRequestSHA512: string =
  ...
  test("integrity check for request signed with RSA-SHA1", () => {
    const [verified, verifiedData] = libsaml.verifySignature(
      _falseDecodedRequestSHA1,
      {
        metadata: SPMetadata,
        signatureAlgorithm: signatureAlgorithms.RSA_SHA1,
      },
  ...
  test("verify a XML signature with metadata but with rolling certificate", () => {
    const responseSignedByCert1 = String(
      readFileSync("./test/misc/response_signed_cert1.xml"),
    );
    const responseSignedByCert2 = String(
      readFileSync("./test/misc/response_signed_cert2.xml"),
    );
    expect(
  ...
  test("verify a XML signature signed by RSA-SHA1 with .cer keyFile", () => {
    ...
    expect(
      libsaml.verifySignature(xml, { keyFile: "./test/key/sp/cert.cer" })[0],
    ).toBe(true);
  });
...
test.skip("metadata with multiple entity descriptors is invalid", () => {
  expect(() => {
    identityProvider({
      ...defaultIdpConfig,
      metadata: readFileSync("./test/misc/multiple_entitydescriptor.xml"),
    });
  }).toThrow();
  try {
    identityProvider({
...
test("get name id format in metadata", () => {
  expect(sp.entityMeta.getNameIDFormat()).toBe(
...
test("contains shared certificate for both signing and encryption in metadata", () => {
  const metadata = idpMetadata(
    readFileSync("./test/misc/idpmeta_share_cert.xml"),
  );
