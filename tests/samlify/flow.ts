...
import { PostBindingContext, SimpleSignBindingContext } from "@robelest/samlify/src/entity";
import util from "@robelest/samlify/src/utility";
...
const parseQueryObject = (value: string) =>
  Object.fromEntries(new URL(value).searchParams.entries()) as Record<string, string>;
...
const parseRedirectUrlContextCallBack = (_context: string) => {
  const originalQuery = parseQueryObject(_context);
  const _SAMLResponse = originalQuery.SAMLResponse;
  const _Signature = originalQuery.Signature;
  const _SigAlg = originalQuery.SigAlg;
  const { Signature: _ignored, ...unsignedQuery } = originalQuery;
  const _octetString = Object.keys(unsignedQuery)
    .map((q) => q + "=" + encodeURIComponent(unsignedQuery[q] as string))
    .join("&");
...
const buildSimpleSignOctetString = (
  type: string,
  context: string,
...
const noSignedIdpMetadata = fixtureRead("misc/idpmeta_nosign.xml").toString().trim();
const spmetaNoAssertSign = fixtureRead("misc/spmeta_noassertsign.xml").toString().trim();
...
const idpNoEncrypt = identityProvider({
  ...defaultIdpConfig,
...
const spWantLogoutReqSign = serviceProvider({
  ...defaultSpConfig,
  wantLogoutRequestSigned: true,
});
const idpWantLogoutResSign = identityProvider({
  ...defaultIdpConfig,
...
const spWithClockDrift = serviceProvider({
  ...defaultSpConfig,
  clockDrifts: [-2000, 2000],
});
...
test("create login request with redirect binding using default template and parse it", async () => {
  ...
  const originalQuery = parseQueryObject(context);
  const SAMLRequest = originalQuery.SAMLRequest;
  const Signature = originalQuery.Signature;
  const SigAlg = originalQuery.SigAlg;
  const { Signature: _ignored, ...unsignedQuery } = originalQuery;
  const octetString = Object.keys(unsignedQuery)
    .map((q) => q + "=" + encodeURIComponent(unsignedQuery[q] as string))
    .join("&");
  const { samlContent: _, extract } = await idp.parseLoginRequest(sp, "redirect", {
  ...
  expect(extract.nameIDPolicy.format).toBe(
...
test("create login request with post simpleSign binding using default template and parse it", async () => {
  ...
  const { samlContent: _, extract } = await idp.parseLoginRequest(sp, "simpleSign", {
...
test("create login request with post binding using default template and parse it", async () => {
  const {
    ...
    id,
    context: SAMLRequest,
  } = sp.createLoginRequest(idp, "post") as PostBindingContext;
  expect(typeof id).toBe("string");
  expect(typeof SAMLRequest).toBe("string");
  expect(typeof entityEndpoint).toBe("string");
  expect(type).toBe("SAMLRequest");
  const { extract } = await idp.parseLoginRequest(sp, "post", {
    body: { SAMLRequest },
  });
  expect(extract.issuer).toBe("https://sp.example.org/metadata");
  expect(typeof extract.request.id).toBe("string");
...
test("signed in sp is not matched with the signed notation in idp with post request", () => {
  const _idp = identityProvider({
    ...defaultIdpConfig,
    metadata: noSignedIdpMetadata,
  });
  try {
...
test("create login request with redirect binding using [custom template]", () => {
  ...
  const { id, context } = _sp.createLoginRequest(idp, "redirect", (template: string) => {
    return {
      id: "exposed_testing_id",
      context: template, // all the tags are supposed to be replaced
    };
  });
  expect(id === "exposed_testing_id" && isString(context)).toBe(true);
});
...
test("create login request with post binding using [custom template]", () => {
  ...
  const { id, context, entityEndpoint, type, relayState } = _sp.createLoginRequest(
    idp,
    "post",
    (template: string) => {
      return {
...
test("create login request with post simpleSign binding using [custom template]", () => {
  ...
  const { id, context, entityEndpoint, type, relayState, signature, sigAlg } =
    _sp.createLoginRequest(idp, "simpleSign", (template: string) => {
      return {
        id: "exposed_testing_id",
        context: template, // all the tags are supposed to be replaced
      };
...
test("create login response with undefined binding", async () => {
  ...
  await expect(
    idp.createLoginResponse(
      ...
      "undefined",
      user,
      createTemplateCallback(idp, sp, binding.post, user),
    ),
  ).rejects.toThrow();
  const error = await idp
    .createLoginResponse(
      sp,
      {},
      "undefined",
...
test("create redirect login response", async () => {
  ...
  const { id, context } = await idp.createLoginResponse(
    ...
    "redirect",
    user,
    createTemplateCallback(idp, sp, binding.redirect, user),
    undefined,
    "relaystate",
  );
...
test("create post simpleSign login response", async () => {
  const user = { email: "user@esaml2.com" };
  const { id, context, entityEndpoint, signature, sigAlg } = (await idp.createLoginResponse(
    sp,
    sampleRequestInfo,
    "simpleSign",
    user,
    createTemplateCallback(idp, sp, binding.simpleSign, user),
    undefined,
    "relaystate",
  )) as SimpleSignBindingContext;
  expect(
...
test("create logout request with post binding", () => {
  const { type, entityEndpoint, id, context } = sp.createLogoutRequest(idp, "post", {
    logoutNameID: "user@esaml2",
  }) as PostBindingContext;
  expect(
    isString(id) && isString(context) && isString(entityEndpoint) && type === "SAMLRequest",
  ).toBe(true);
});

test("create logout request when idp only has one binding", () => {
  const testIdp = identityProvider(oneloginIdpConfig);
  const { id, context } = sp.createLogoutRequest(testIdp, "redirect", {
    logoutNameID: "user@esaml2",
  });
  expect(isString(id) && isString(context)).toBe(true);
});
...
test("create logout response with undefined binding", () => {
  try {
    idp.createLogoutResponse(
      ...
      "undefined",
      "",
      createTemplateCallback(idp, sp, binding.post, {}),
    );
    expect(true).toBe(false);
  } catch (e: any) {
...
test("create logout response with post binding", () => {
  const { type, entityEndpoint, id, context } = idp.createLogoutResponse(
    sp,
    {},
    "post",
    "",
  ...
  expect(
    isString(id) && isString(context) && isString(entityEndpoint) && type === "SAMLResponse",
  ).toBe(true);
});
...
test("send response with signed assertion and parse it", async () => {
  ...
  const { samlContent, extract } = await sp.parseLoginResponse(idpNoEncrypt, "post", {
    body: { SAMLResponse },
  });
  expect(typeof id).toBe("string");
...
test("send response with signed assertion by redirect and parse it", async () => {
  ...
  const { id, context } = await idpNoEncrypt.createLoginResponse(
    ...
    "redirect",
    user,
    createTemplateCallback(idpNoEncrypt, sp, binding.redirect, user),
    undefined,
    "relaystate",
  );
...
test("send response with signed assertion by post simplesign and parse it", async () => {
  ...
  const { samlContent, extract } = await sp.parseLoginResponse(idpNoEncrypt, "simpleSign", {
...
test("send response with signed assertion + custom transformation algorithms by redirect and parse it", async () => {
  ...
  const { id, context } = await idpNoEncrypt.createLoginResponse(
    ...
    "redirect",
    user,
    createTemplateCallback(idpNoEncrypt, signedAssertionSp, binding.redirect, user),
    undefined,
    "relaystate",
  );
  const query = parseQuery(context);
...
test("send response with [custom template] signed assertion and parse it", async () => {
  ...
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const { samlContent, extract } = await sp.parseLoginResponse(idpcustomNoEncrypt, "post", {
    body: { SAMLResponse },
  });
  expect(typeof id).toBe("string");
  expect(samlContent.startsWith("<samlp:Response")).toBe(true);
...
test("send response with [custom template] signed assertion by post simpleSign and parse it", async () => {
  ...
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const octetString = buildSimpleSignOctetString(type, SAMLResponse, sigAlg, relayState, signature);
  const { samlContent, extract } = await sp.parseLoginResponse(idpcustomNoEncrypt, "simpleSign", {
    body: { SAMLResponse, Signature: signature, SigAlg: sigAlg },
    octetString,
  });
  expect(typeof id).toBe("string");
...
test("send response with signed message and parse it", async () => {
  ...
  const { id, context: SAMLResponse } = await idpNoEncrypt.createLoginResponse(
    ...
    "post",
    user,
    createTemplateCallback(idpNoEncrypt, spNoAssertSign, binding.post, user),
  );
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const { samlContent, extract } = await spNoAssertSign.parseLoginResponse(idpNoEncrypt, "post", {
    body: { SAMLResponse },
  });
  expect(typeof id).toBe("string");
  expect(samlContent.startsWith("<samlp:Response")).toBe(true);
...
test("send login response with signed assertion + signed message and parse it", async () => {
  ...
  const { id, context: SAMLResponse } = await idpNoEncrypt.createLoginResponse(
    ...
    "post",
    user,
    createTemplateCallback(idpNoEncrypt, spWantMessageSign, binding.post, user),
  );
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
...
test("send login response with encrypted non-signed assertion and parse it", async () => {
  ...
  const { id, context: SAMLResponse } = await idp.createLoginResponse(
    ...
    "post",
    user,
    createTemplateCallback(idp, spNoAssertSign, binding.post, user),
  );
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const { samlContent, extract } = await spNoAssertSign.parseLoginResponse(idp, "post", {
    body: { SAMLResponse },
  });
  expect(typeof id).toBe("string");
  expect(samlContent.startsWith("<samlp:Response")).toBe(true);
...
test("send login response with [custom template] and encrypted signed assertion and parse it", async () => {
  ...
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const { samlContent, extract } = await sp.parseLoginResponse(idpcustom, "post", {
    body: { SAMLResponse },
  });
  expect(typeof id).toBe("string");
  expect(samlContent.startsWith("<samlp:Response")).toBe(true);
...
test("send login response with encrypted signed assertion + signed message and parse it", async () => {
  ...
  const { id, context: SAMLResponse } = await idp.createLoginResponse(
    ...
    "post",
    user,
    createTemplateCallback(idp, spWantMessageSign, binding.post, user),
  );
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const { samlContent, extract } = await spWantMessageSign.parseLoginResponse(idp, "post", {
    body: { SAMLResponse },
  });
...
test("send login response with [custom template] encrypted signed assertion + signed message and parse it", async () => {
  ...
  // receiver (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const { samlContent, extract } = await spWantMessageSign.parseLoginResponse(idpcustom, "post", {
    body: { SAMLResponse },
  });
  expect(typeof id).toBe("string");
  expect(samlContent.startsWith("<samlp:Response")).toBe(true);
...
test("idp sends a redirect logout request without signature and sp parses it", async () => {
  const { id, context } = idp.createLogoutRequest(sp, "redirect", {
  ...
  expect(typeof context).toBe("string");
  const originalQuery = parseQueryObject(context);
  const SAMLRequest = encodeURIComponent(originalQuery.SAMLRequest as string);
  let result;
  const { samlContent, extract } = (result = await sp.parseLogoutRequest(idp, "redirect", {
    query: { SAMLRequest },
  }));
...
test("idp sends a redirect logout request with signature and sp parses it", async () => {
  const { id, context } = idp.createLogoutRequest(spWantLogoutReqSign, "redirect", {
    logoutNameID: "user@esaml2.com",
  });
  const query = parseQuery(context);
  expect(query!.includes("SAMLRequest=")).toBe(true);
  ...
  const Signature = originalQuery.Signature;
  const SigAlg = originalQuery.SigAlg;
  const { Signature: _ignoredLogout, ...unsignedLogoutQuery } = originalQuery;
  const octetString = Object.keys(unsignedLogoutQuery)
    .map((q) => q + "=" + encodeURIComponent(unsignedLogoutQuery[q] as string))
    .join("&");
  const { extract } = await spWantLogoutReqSign.parseLogoutRequest(idp, "redirect", {
    query: { SAMLRequest, Signature, SigAlg },
    octetString,
  });
  expect(extract.nameID).toBe("user@esaml2.com");
...
test("idp sends a post logout request without signature and sp parses it", async () => {
  const { type, entityEndpoint, id, context } = idp.createLogoutRequest(sp, "post", {
    logoutNameID: "user@esaml2.com",
  }) as PostBindingContext;
  expect(typeof id).toBe("string");
  expect(typeof context).toBe("string");
  expect(typeof entityEndpoint).toBe("string");
  expect(type).toBe("SAMLRequest");
  const { extract } = await sp.parseLogoutRequest(idp, "post", {
...
test("idp sends a post logout request with signature and sp parses it", async () => {
  const { type, entityEndpoint, id, context } = idp.createLogoutRequest(
    spWantLogoutReqSign,
    "post",
    { logoutNameID: "user@esaml2.com" },
  ) as PostBindingContext;
  ...
  expect(typeof entityEndpoint).toBe("string");
  expect(type).toBe("SAMLRequest");
  const { extract } = await spWantLogoutReqSign.parseLogoutRequest(idp, "post", {
    body: { SAMLRequest: context },
  });
  expect(extract.nameID).toBe("user@esaml2.com");
  expect(extract.issuer).toBe("https://idp.example.com/metadata");
...
test("sp sends a post logout response without signature and parse", async () => {
  ...
  const { extract } = await idp.parseLogoutResponse(sp, "post", {
    body: { SAMLResponse },
  });
  expect(extract.signature).toBe(null);
  expect(extract.issuer).toBe("https://sp.example.org/metadata");
...
test("sp sends a post logout response with signature and parse", async () => {
  const { context: SAMLResponse } = sp.createLogoutResponse(
    ...
    "post",
    "",
    createTemplateCallback(idpWantLogoutResSign, sp, binding.post, {}),
  ) as PostBindingContext;
  const { samlContent: _, extract } = await idpWantLogoutResSign.parseLogoutResponse(sp, "post", {
    body: { SAMLResponse },
  });
  expect(typeof extract.signature).toBe("string");
  expect(extract.issuer).toBe("https://sp.example.org/metadata");
...
test("send login response with encrypted non-signed assertion with EncryptThenSign and parse it", async () => {
  ...
  const { id, context: SAMLResponse } = await idpEncryptThenSign.createLoginResponse(
    ...
    "post",
    user,
    createTemplateCallback(idpEncryptThenSign, spNoAssertSignCustomConfig, binding.post, user),
    true,
  );
...
test("Customize prefix (saml2) for encrypted assertion tag", async () => {
  ...
  const { context: SAMLResponse } = await idpCustomizePfx.createLoginResponse(
    sp,
    sampleRequestInfo,
    "post",
    user,
    createTemplateCallback(idpCustomizePfx, sp, binding.post, user),
  );
  expect((utility.base64Decode(SAMLResponse) as string).includes("saml2:EncryptedAssertion")).toBe(
    true,
  );
  await sp.parseLoginResponse(idpCustomizePfx, "post", {
    body: { SAMLResponse },
  });
});
...
test("Customize prefix (default is saml) for encrypted assertion tag", async () => {
  ...
  expect((utility.base64Decode(SAMLResponse) as string).includes("saml:EncryptedAssertion")).toBe(
    true,
  );
  await sp.parseLoginResponse(idp, "post", {
    body: { SAMLResponse },
  });
});
...
test("avoid malformatted response with redirect binding", async () => {
  // sender (caution: only use metadata and public key when declare pair-up in oppoent entity)
  const user = { email: "user@email.com" };
  const { context } = await idpNoEncrypt.createLoginResponse(
    sp,
    sampleRequestInfo,
    "redirect",
    user,
  ...
  const originalQuery = parseQueryObject(context);
  const SAMLResponse = originalQuery.SAMLResponse;
  const signature = originalQuery.Signature;
  const sigAlg = originalQuery.SigAlg;
  ...
  const octetString =
    "SAMLResponse=" +
    encodeURIComponent(utility.base64Encode(utility.deflateString(attackResponse))) +
    "&SigAlg=" +
    encodeURIComponent(sigAlg as string);
  try {
    await sp.parseLoginResponse(idpNoEncrypt, "redirect", {
...
test("avoid malformatted response with simplesign binding", async () => {
  ...
  const octetString = buildSimpleSignOctetString(type, SAMLResponse, sigAlg, relayState, signature);
  try {
    await sp.parseLoginResponse(idpNoEncrypt, "simpleSign", {
      body: {
...
test("should use signed contents in signature wrapped response - case 2", async () => {
  ...
  const wrappedResponse = Buffer.from(xmlWrapped).toString("base64");
  const { extract } = await sp.parseLoginResponse(idpNoEncrypt, "post", {
    body: { SAMLResponse: wrappedResponse },
  });
  expect(extract.nameID).toBe("user@esaml2.com");
});
...
test("should reject response with duplicate signed reference IDs", async () => {
  ...
  try {
    await sp.parseLoginResponse(idpNoEncrypt, "post", {
      body: {
        SAMLResponse: Buffer.from(xmlWithDuplicateId).toString("base64"),
      },
    });
    expect(true).toBe(false);
  } catch (e: any) {
...
test("should reject signature with quoted reference URI", async () => {
  ...
  const xml = Buffer.from(SAMLResponse, "base64").toString();
  const xmlWithBadReferenceUri = xml.replace(/Reference URI="#([^"]+)"/, `Reference URI="#$1'"`);

  try {
    await sp.parseLoginResponse(idpNoEncrypt, "post", {
      body: {
        SAMLResponse: Buffer.from(xmlWithBadReferenceUri).toString("base64"),
      },
    });
    expect(true).toBe(false);
  } catch (e: any) {
...
test("should throw two-tiers code error when the response by redirect does not return success status", async () => {
  try {
    ...
    const octetString =
      "SAMLResponse=" + encodeURIComponent(SAMLResponse) + "&SigAlg=" + encodedSigAlg;
    await sp.parseLoginResponse(idpNoEncrypt, "redirect", {
...
test.sequential("should throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response by simpleSign without clock drift setup", async () => {
  ...
  try {
    ...
    // simulate the time on client side when response arrives after 5.1 sec
    tk.freeze(fiveMinutesOneSecLater);
    await sp.parseLoginResponse(idp, "simpleSign", {
      body: { SAMLResponse, Signature: signature, SigAlg: sigAlg },
      octetString,
    });
    // test failed, it shouldn't happen
...
test.sequential("should not throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response with clock drift setup", async () => {
  ...
  try {
    const { context: SAMLResponse } = await idp.createLoginResponse(
      ...
      "post",
      user,
      createTemplateCallback(idp, spWithClockDrift, binding.post, user),
    );
    // simulate the time on client side when response arrives after 5.1 sec
    tk.freeze(fiveMinutesOneSecLater);
    await spWithClockDrift.parseLoginResponse(idp, "post", {
      body: { SAMLResponse },
    });
    expect(true).toBe(true);
  } catch {
...
test.sequential("should not throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response by redirect with clock drift setup", async () => {
  ...
  try {
    const { context: SAMLResponse } = await idp.createLoginResponse(
      ...
      "redirect",
      user,
      createTemplateCallback(idp, spWithClockDrift, binding.redirect, user),
      undefined,
      "",
    );
    // simulate the time on client side when response arrives after 5.1 sec
    ...
    await spWithClockDrift.parseLoginResponse(
      idp,
      "redirect",
      parseRedirectUrlContextCallBack(SAMLResponse),
    );
    expect(true).toBe(true);
  } catch {
    // test failed, it shouldn't happen
...
test.sequential("should not throw ERR_SUBJECT_UNCONFIRMED for the expired SAML response by simpleSign with clock drift setup", async () => {
  ...
  try {
    const {
      ...
      "simpleSign",
      user,
      createTemplateCallback(idp, spWithClockDrift, binding.simpleSign, user),
      undefined,
      "relaystate",
    );
    const octetString = buildSimpleSignOctetString(
    ...
    // simulate the time on client side when response arrives after 5.1 sec
    tk.freeze(fiveMinutesOneSecLater);
    await spWithClockDrift.parseLoginResponse(idp, "simpleSign", {
      body: { SAMLResponse, Signature: signature, SigAlg: sigAlg },
      octetString,
    });
    expect(true).toBe(true);
