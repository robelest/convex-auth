...
test("SCIM → Convex: direct SCIM server protocol validation", async () => {
  ...
  expect(
    auditEvents.some(
      (event) => event.eventType === "group.sso.scim.configured" && event.status === "success",
    ),
  ).toBe(true);
  expect(
    auditEvents.some(
      (event) => event.eventType === "group.sso.scim.user.created" && event.subjectId === userId,
    ),
  ).toBe(true);
  expect(
    auditEvents.some(
      (event) => event.eventType === "group.sso.scim.user.updated" && event.subjectId === userId,
    ),
  ).toBe(true);
  expect(
    auditEvents.some(
      (event) => event.eventType === "group.sso.scim.user.deleted" && event.subjectId === userId,
    ),
  ).toBe(true);
  expect(
    auditEvents.some(
      (event) =>
        event.eventType === "group.sso.scim.group.created" && event.subjectId === scimGroupId,
    ),
  ).toBe(true);
  expect(
    auditEvents.some(
      (event) =>
        event.eventType === "group.sso.scim.group.updated" && event.subjectId === scimGroupId,
    ),
  ).toBe(true);
  ...
  const webhookDeliveries = (await groupWebhookDeliveryListRpc(convexClient, convexUserToken, {
    connectionId,
    limit: 50,
  })) as GroupWebhookDelivery[];
  expect(
    webhookDeliveries.some(
      (delivery) => delivery.eventType === "group.sso.scim.configured" && delivery.auditEventId,
    ),
  ).toBe(true);
  expect(
    webhookDeliveries.some(
      (delivery) => delivery.eventType === "group.sso.scim.user.created" && delivery.auditEventId,
    ),
  ).toBe(true);
  expect(
    webhookDeliveries.some(
      (delivery) => delivery.eventType === "group.sso.scim.group.updated" && delivery.auditEventId,
    ),
  ).toBe(true);
}, 60_000);
