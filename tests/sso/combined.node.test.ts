...
async function configureScimAndProvisionUser(args: {
  ...
  const scimUser = await requestJson<ScimUser>(
    `${args.convexSiteUrl}/api/auth/connections/${args.connectionId}/scim/v2/Users`,
    {
      method: "POST",
      headers: {
...
test("SCIM + OIDC reuses provisioned userId", async () => {
  ...
  const connectionCallbackUrl = `${convexSiteUrl}/api/auth/connections/${connectionId}/oidc/callback`;
...
test("SCIM + SAML reuses provisioned userId", async () => {
  ...
  const convexAcsUrl = `${convexSiteUrl}/api/auth/connections/${connectionId}/saml/acs`;
  const convexEntityId = `${convexSiteUrl}/api/auth/connections/${connectionId}/saml/metadata`;
  const spMetadataXml = [
    `<?xml version="1.0"?>`,
