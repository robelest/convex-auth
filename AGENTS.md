...
**Bun** workspaces with ESM (`"type": "module"`) throughout.
...
- The `examples/tanstack` dir has its own Prettier config (no semis, single
  quotes) — do not mix.
...
- Use `throwAuthError(code)` for auth-domain errors — do NOT throw raw
  `ConvexError` directly.
...
- Catalog versions: `"catalog:"` in sub-packages resolves from root
